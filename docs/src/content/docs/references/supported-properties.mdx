...
### Radial Gradient Logical Positions <Badge text="NEW" variant="tip" />

**⚠️ Experimental Feature** - Enhanced support for radial gradient logical positioning.

```css
/* Simple logical positions */
.element {
...
**Advanced mixed positioning:**
```css
/* Mixed logical and physical positions */
.element {
...
**Supported logical positions:**
- `at inline-start` → direction-aware `at left`/`at right`
- `at inline-end` → direction-aware `at right`/`at left`
- `at block-start` → `at top` (direction-independent)
- `at block-end` → `at bottom` (direction-independent)
- Mixed positions like `at inline-start top`, `at bottom inline-end`, `at 50% inline-start` etc.

**Compound directions:**
```css
/* Compound logical positions */
.element {
...
### Repeating Gradients <Badge text="NEW" variant="tip" />

**⚠️ Experimental Feature** - Full support for repeating gradient logical directions.
...
**All logical directions are supported:**
- `repeating-linear-gradient` with `to inline-start/end`, `to block-start/end`
